...
## Si no ves cambios en GitHub

Eso pasa cuando los commits están en tu rama local (`work`) pero **no** en `main` remoto.

Publica todo a `main` con:

```bash
bash scripts/publish_agent_changes.sh https://github.com/dleiva98/agriprice-cr.git
```

Este script aplica (cherry-pick) los commits del agente y hace `git push` a `main`.
