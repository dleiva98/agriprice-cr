...
1. **Fuentes de datos**
   - Precios agrícolas: PIMA/CENADA (scraping + carga manual de respaldo).
   - Clima: Open-Meteo (API histórica gratis).
   - Economía: World Bank API + BCCR (cuando tengas token).
   - Política/mercado (plus): indicadores de riesgo país, inflación, tipo de cambio, noticias (fase 2).
...
- [ ] Ejecutar descarga de clima (Open-Meteo).
- [ ] Ejecutar descarga macro (World Bank).
- [ ] Probar scraping de precios (PIMA).
- [ ] Si falla scraping: cargar CSV/PDF manuales en `data/raw/precios/` y continuar.
...
No subir datos sensibles ni secretos. Usa GitHub Secrets para claves.
